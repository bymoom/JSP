...
<!-- DataFormat, numberFormat 사용하기 위해... -->
...
<script>
	$('#searchBtn').on('click', function(e){
		var jobForm = $('#jobForm');
		jobForm.find("[name='page']").val(1); //3번 페이지 보다가 검색을 하면 1번 페이지로 보내기
		jobForm.find("[name='searchType']").val($('select[name="searchType"]').val());
		jobForm.find("[name='keyword']").val($('input[name="keyword"]').val());

		//alert(jobForm.serialize()); //form을 ajax로 보낼때 사용(url에  keyword=kim이라고 쓴 것처럼 파라미터로 날라감)
